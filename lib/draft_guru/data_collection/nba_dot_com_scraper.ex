if Code.ensure_loaded?(Wallaby.DSL) do
  ...
  defmodule DraftGuru.NBADotComScraper do
    ...
    @doc """
    Placeholder module for when Wallaby is not loaded -- would happen in a prod environment
    """

    def scrape_combine_data_for_section(combine_section) do
      Logger.warning("NBADotComScraper called in environment where Wallaby is not available (MIX_ENV=#{Mix.env()}). Section: #{combine_section}")
      # Return an appropriate value indicating failure or inaction
      {:error, :scraping_disabled_in_prod}
    end
  end
end
